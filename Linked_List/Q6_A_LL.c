...
int moveMaxToFront(ListNode **ptrHead)
{
	...
	ListNode *maxPrevPtr = NULL, *prevPtr = NULL, *curPtr = *ptrHead;
	int maxValue = -INT_MAX;
	while (curPtr != NULL)
	{
		if (curPtr->item > maxValue)
		{
			maxValue = curPtr->item;
			maxPrevPtr = prevPtr;
		}
		prevPtr = curPtr;
		curPtr = curPtr->next;
	}
	if (maxPrevPtr == NULL && maxValue != -INT_MAX) // 가장 앞이 MAX인 경우
		return maxValue;

	curPtr = maxPrevPtr->next;
	maxPrevPtr->next = maxPrevPtr->next->next;
	curPtr->next = *ptrHead;
	*ptrHead = curPtr;
	return maxValue;
}
