...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
	...
	if (ll2_size == 0 || ll1_size == 0)
	{
		return;
	}
	int min_size = ll1_size > ll2_size ? ll2_size : ll1_size;

	ListNode *l1_node, *l2_node, *next_l2_node;
	l1_node = ll1->head;
	next_l2_node = ll2->head; // for temp pointer save
	l2_node = NULL;

	int count = 0;
	while (l1_node != NULL && next_l2_node != NULL)
	{
		count++;
		l2_node = next_l2_node;
		next_l2_node = l2_node->next;
		l2_node->next = l1_node->next;
		l1_node->next = l2_node;
		l1_node = l2_node->next;
	}
	ll2->head = next_l2_node;
	ll2->size -= count;
	ll1->size -= count;
}
