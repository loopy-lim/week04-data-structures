...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	...
	if (length < 1)
		return;
	int leftLength = length / 2 + (length % 2 == 0 ? 0 : 1);
	int rightLength = length / 2;

	ListNode *frontListNode = resultFrontList->head,
					 *backListNode = resultBackList->head,
					 *cur = ll->head;

	for (int i = 0; i < leftLength - 1; i++)
		cur = cur->next;

	resultFrontList->size = leftLength;
	resultBackList->size = rightLength;
	ll->size = 0;
